...
static void paser_msg_responseLine(char data);
static void paser_msg_responseHead(char data);
static void paser_msg_responseBody(char data);

// 全局变量
static HTTP_RESPONSE_TYPEDEF http_response;
static HTTP_MSG_TYPEDEF m_http_msg;
static int msg_index = 0;
#define HTTP_MAX_TEMP 500
static char g_msg_responseLine_temp[HTTP_MAX_TEMP];
...
/**
* @ Function Name : paser_msg_responseLine
* @ Author        : ygl
* @ Brief         : 解析报文响应行
* @ Date          : 2019.03.05
...
static void paser_msg_responseLine(char data)
{
	#define RES_MAX_TEMP 30
	static char responseLine_temp[RES_MAX_TEMP];
	static int responseLine_index = 0;
	HTTP_RESPONSE_LINE_TYPEDEF http_response_line;

	
	if (responseLine_index > RES_MAX_TEMP){
		m_printf("res error decode:%c\r\n",data);
...
/**
* @ Function Name : paser_msg_responseHead
* @ Author        : ygl
* @ Brief         : 解析报文解析头
* @ Date          : 2019.03.05
* @ Input         : char data  将要解析的数据
...
static void paser_msg_responseHead(char data)
{
	...
	if (responseLine_index > HTTP_MAX_TEMP){
		m_printf("head error:%d ss:%d decode:%c\r\n",responseLine_index,MAX_TEMP,data);
		responseLine_index = 0;
		return;
	}else{
	...
	if (data == '\n')
	{
		...
		fun = paser_msg_responseBody;
			...
			if (strcmp(head_tmep,"Content-Length") == 0){
              m_http_msg.len = atoi(http_response_head.head_list[http_response_head_index].val);	
			  			// m_printf("--*head*%s--\r\n", head_tmep);
						// m_printf("--*val*%s--\r\n", head_val);
						content_type = 1;
			}
...
/**
* @ Function Name : paser_msg_responseBody
* @ Author        : ygl
...
void parser_init(void)
{
	msg_index = 0;
	content_type = 0;
	fun = paser_msg_responseLine;
}


/**
* @ Function Name : paser_msg
* @ Author        : ygl
* @ Brief         : 解析消息,由上层调动
* @ Date          : 2019.03.05
* @ Input         : char data 将要解析的数据
...
/**
* @ Function Name : parser_regist_http_msg_callback
* @ Author        : ygl
* @ Brief         : 注册https消息接收函数
* @ Date          : 2019.03.05
* @ Input         : 注册回调
...
/**
* @ Function Name : parser_regist_printCallback
* @ Author        : ygl
* @ Brief         : 注册打印函数
* @ Date          : 2019.03.05
...
/**
* @ Function Name : parser_regist_msg_callback
* @ Author        : ygl
* @ Brief         : 注册消息接收
* @ Date          : 2019.03.05
...
/**
* @ Function Name : parser_regist_http_head_callback
* @ Author        : ygl
* @ Brief         : 注册http头部接收
* @ Date          : 2019.03.05
